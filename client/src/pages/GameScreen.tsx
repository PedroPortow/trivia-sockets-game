...
function GameScreen() {
  ...
  const [currentQuestion, setCurrentQuestion] = useState<Question | null>(currentRoom?.questions?.[0] || null)
  ...
  const onTimerFinish = () => {
    ...
    if (currentQuestionIndex === (currentRoom?.questions?.length || 0) - 1) {
      navigate(`/rooms/${currentRoom?.id}/game/result`)
      return
    }
      
    timerRef.current?.reset()
    setCurrentQuestion(currentRoom?.questions?.[(currentQuestionIndex || 0) + 1] || null)
    setCurrentAnswerId(null)
  }

  const currentQuestionIndex = useMemo(() => {
    return currentRoom?.questions?.findIndex(q => q.id === currentQuestion?.id)
  }, [currentRoom?.questions, currentQuestion?.id])

  return (
    <div className="min-h-dvh pt-16 items-center justify-center flex flex-col gap-16">
      <p>Pergunta {(currentQuestionIndex || 0) + 1} de {currentRoom?.questions?.length || 0}</p>
